...
Each [Read Replica](/docs/guides/platform/read-replicas) is a dedicated database. You are charged for its resources, which are:
...
If the primary database has a configured IPv4 address, its Read Replicas are also assigned one, with charges for each. Read the [Manage IPv4 usage guide](/docs/guides/platform/manage-your-usage/ipv4) for details on how we calculate charges.
