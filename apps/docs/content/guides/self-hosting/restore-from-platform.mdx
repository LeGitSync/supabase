...
- [Supabase CLI](/docs/guides/local-development/cli/getting-started) installed
...
### Permission errors with `supabase_admin`

If you see errors like `must be able to SET ROLE "supabase_admin"` or `ALTER ... OWNER TO "supabase_admin"` during restore, this is because the non-superuser `postgres` role doesn't have permission to execute `ALTER ... OWNER TO "supabase_admin"` statements that appear in the dump for Supabase-internal schemas (`auth`, `storage`, `realtime`, `extensions`, etc.).

Since you're restoring to a fresh self-hosted instance, these schemas are already set up during Docker initialization and the ownership statements from the dump are unnecessary. Your user objects in the `public` schema are owned by `postgres` on both platform and self-hosted, so they restore without issues.

**Workaround:** Before restoring, comment out the ownership statements from `schema.sql`:
