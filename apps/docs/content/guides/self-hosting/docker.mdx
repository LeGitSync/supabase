...
By default all files are stored locally on the server. You can connect Storage to an S3-compatible backend (AWS S3, MinIO, Cloudflare R2) and enable the S3 protocol endpoint for tools like `rclone`.

See the [Configure S3 Storage](/docs/guides/self-hosting/configure-storage) guide for detailed setup instructions.
